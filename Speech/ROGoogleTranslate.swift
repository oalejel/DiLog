...
public struct ROGoogleTranslateParams {
    ...
    public init(source:String, target:String, text:String) {
        self.source = acceptableLanguageCode(inCode: source)
        self.target = acceptableLanguageCode(inCode: target)
        self.text = text
    }
    
    func acceptableLanguageCode(inCode: String) -> String {
        var outCode = ""
        for c in inCode.characters {
            if(c == "-") {
                break
            }
            outCode.append(c)
...
open class ROGoogleTranslate {
    ...
    public var apiKey = "AIzaSyB2H1wm89e_Bcb3O1iNGmLzW9v7dP96UKg"
